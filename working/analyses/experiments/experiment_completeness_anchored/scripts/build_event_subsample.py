...
def parse_args(argv=None):
    ...
    ap.add_argument("--block_index", type=int, default=None,
                    help="Take the k-th DISJOINT contiguous block of n_events instead "
                         "of a stratified stride. Disjoint blocks give independent GW "
                         "realisations from one parent set, so their scatter measures "
                         "GW noise while any common offset is systematic (the catalog "
                         "and injection set are shared and do NOT average down).")
...
def main(argv=None):
    ...
    with h5py.File(args.in_path, "r") as f:
        ...
        if "host_type" in f:
            ...
            # Single-catalog (K=1) mock: no host split to preserve, so treat every
            # event as one stratum. Events are i.i.d., so an even stride is a fair
            # subsample.
            host_type = np.zeros(nobs, dtype=np.int8)
            print("[note] no host_type dataset (K=1 mock); single-stratum subsample")

        if args.block_index is not None:
            lo = args.block_index * args.n_events
            hi = lo + args.n_events
            if hi > nobs:
                raise SystemExit(f"[fatal] block {args.block_index} exceeds nobs {nobs}")
            sel = np.arange(lo, hi)
        else:
    ...
    new_attrs.update({
        ...
        "subsample_parent_truth_alpha_agn": (attrs["n_host_agn"] / nobs
                                            if "n_host_agn" in attrs else 0.0),
