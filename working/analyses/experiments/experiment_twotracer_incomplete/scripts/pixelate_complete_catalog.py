...
def parse_args(argv=None):
    ...
    ap.add_argument("--mag_limit", type=float, default=None,
                    help="ISOTROPIC flux limit: keep hosts with app_mag < this. Gives a "
                         "physically-shaped, declining C(z) as a CONSEQUENCE rather than "
                         "an input, with no footprint and no hard z cut, so the "
                         "incompleteness the inference must model is isotropic by "
                         "construction.")
    ap.add_argument("--completeness_z_ref", type=float, default=0.27,
                    help="Report completeness within this redshift (the GW horizon), "
                         "which is the physically meaningful knob — completeness over "
                         "the full catalog depth is dominated by hosts no event can have.")
...
def main(argv=None):
    ...
    with h5py.File(args.complete_catalog, "r") as f:
        keys = list(f.keys())
        app_mag = (np.asarray(f["app_mag"][:], dtype=float)
                   if "app_mag" in f else None)
    ...
    completeness = {"mag_limit": args.mag_limit}
    if args.mag_limit is not None:
        if app_mag is None:
            raise SystemExit("[fatal] --mag_limit needs an app_mag dataset")
        zref = args.completeness_z_ref
        keep = app_mag < args.mag_limit
        in_h = z <= zref
        completeness["within_z_ref"] = float(np.mean(keep[in_h])) if in_h.any() else None
        completeness["all_z"] = float(np.mean(keep))
        completeness["z_ref"] = zref
        # C(z) in horizon bins, so the shape is on record, not just the scalar.
        edges = np.linspace(0.0, zref, 7)
        cz = []
        for lo, hi in zip(edges[:-1], edges[1:]):
            b = (z > lo) & (z <= hi)
            cz.append(float(np.mean(keep[b])) if b.any() else None)
        completeness["C_of_z_bins"] = {"edges": edges.tolist(), "C": cz}
        ra, dec, z, app_mag = ra[keep], dec[keep], z[keep], app_mag[keep]
        print(f"  mag_limit {args.mag_limit}: {z.size:,} of {n_before:,} kept | "
              f"C(z<={zref}) = {100*completeness['within_z_ref']:.1f}%  "
              f"all-z {100*completeness['all_z']:.1f}%")
        print(f"  C(z) in {len(cz)} bins to z={zref}: "
              + " ".join("--" if c is None else f"{c:.2f}" for c in cz))
    ...
    with h5py.File(out, "w") as f:
        ...
        f.attrs["completeness_json"] = json.dumps(completeness, default=str)
        f.attrs["completeness"] = (
            "complete (no cuts)" if args.mag_limit is None else
            f"isotropic flux limit app_mag < {args.mag_limit}; "
            f"C(z<={completeness['z_ref']}) = {completeness['within_z_ref']:.4f}")
    print(f"Wrote {out}")
