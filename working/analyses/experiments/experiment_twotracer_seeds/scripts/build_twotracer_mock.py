...
def parse_args(argv=None):
    ...
    ap.add_argument("--z_error", type=float, default=0.003,
                    help="Catalog redshift uncertainty; must be matched to the PE "
                         ...
                         "spectroscopic value.")
    ap.add_argument("--detection_data", choices=["true", "observed"], default="true",
                    help="'true' (default, historical): gmd._network_snr on true "
                         "params with an independent projection latent, PE from a "
                         "separate noise draw (reproduces the pre-fix runs "
                         "bit-for-bit).  'observed': PR #334/#335 fixed generator "
                         "-- one measurement per source, detection thresholds ITS "
                         "SNR, the posterior conditions on that same record "
                         "(use_recorded_observation), and sigma_ang is derived "
                         "SEQUENTIALLY from the observed masses/distance instead "
                         "of the latent true parameters.")
    ap.add_argument("--snr_ref", type=float, default=11.5,
                    help="Amplitude scale of the observed-data detection statistic "
                         "(ignored for --detection_data true, which pins 11.5). "
                         "The projection latent is dropped in observed mode, so "
                         "snr_ref must be recalibrated; 6.278 reproduces the "
                         "true-arm detected fraction on this catalog family "
                         "(matched-mock campaign calibration).")
...
def main(argv=None):
    ...
    # The observed-mode kwargs are only passed when requested, so the default
    # call remains byte-compatible with the pre-fix worktree's gmd signature.
    draw_kwargs = {}
    pe_kwargs = {"dL_fractional_uncertainty": args.dL_fractional_uncertainty}
    if args.detection_data == "observed":
        draw_kwargs = {"detection_data": "observed", "snr_ref": args.snr_ref,
                       "pe_kwargs": pe_kwargs}
    parts = []
    for name, cat, n_ev in (("gal", gal, n_gal_ev), ("agn", agn, n_agn_ev)):
        t = gmd._draw_events_until_detected(rng, n_ev, cat, grids, pop,
                                           args.snr_threshold, **draw_kwargs)
    ...
    post_kwargs = {}
    if args.detection_data == "observed":
        # The posterior must condition on the very measurement the detection
        # threshold was applied to (PR #334), whose sigma_ang is the sequential
        # observable width (PR #335).
        post_kwargs = {"use_recorded_observation": True}
    post, obs = gmd._posterior_samples(
        rng, truth, args.nsamp,
        dL_fractional_uncertainty=args.dL_fractional_uncertainty,
        pe_centering="observed", **post_kwargs)
    ...
    with h5py.File(gw_path, "w") as f:
        ...
        f.attrs["detection_shares_noise_with_pe"] = args.detection_data == "observed"
        f.attrs["snr_ref"] = float(args.snr_ref if args.detection_data == "observed"
                                   else 11.5)
        ...
        for k, v in obs.items():
            if k in truth:
                # Observed mode: the recorded measurement already sits in truth
                # (detection stored it; _posterior_samples echoed it back).  It
                # must be the SAME record, or detection and PE have drifted.
                assert np.array_equal(np.asarray(truth[k]), np.asarray(v)), (
                    f"truth[{k!r}] disagrees with the observation the posterior "
                    "conditioned on")
                continue
            g.create_dataset(k, data=v)
    ...
    (out / "twotracer_meta.json").write_text(json.dumps({
        ...
        "snr_ref": args.snr_ref if args.detection_data == "observed" else 11.5,
