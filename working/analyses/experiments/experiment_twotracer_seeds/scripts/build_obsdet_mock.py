...
def observe(rng, m1det, m2det, chi, dl, ra, dec, sigma_dl, sig_m1_frac,
    ...
    return {
        "dL": dl * np.exp(sigma_dl * rng.normal(size=n)),
        "m1det": np.clip(rng.normal(m1det, sig_m1), 2.0, None),
        "m2det": np.clip(rng.normal(m2det, sig_m2), 1.0, None),
