...
def draw_injections(gmd, ndraw, seed, grids, ddldz_grid, pop, pmaps,
                    ...
                    zmax, verbose=True, detection_data="true", snr_ref=11.5,
                    pe_kwargs=None):
    ...
    while n_proposed < ndraw:
        ...
        # ---- detection: THE MOCK'S OWN RULE ---------------------------------
        if detection_data == "observed":
            # PR #334/#335 fixed rule: one measurement per proposal, threshold
            # ITS SNR.  Stored coordinates and pdraw stay the TRUE parameters
            # (mu is an integral over true parameters; only the detection
            # decision sees the noise), exactly as gmd's own selection batch
            # does in this mode.  sigma_ang never enters the statistic.
            pk = dict(pe_kwargs or {})
            det, _obs = gmd._detect_on_observation(
                ...
                pk.get("chieff_uncertainty", 0.08),
                pk.get("sky_uncertainty_deg"))
        else:
            # historical rule: noisy network SNR with the projection latent
...
def write_file(out_path, sel, args, f_pop, f_unif, tgt_weights, pmaps, gmd, meta):
    ...
    with h5py.File(out_path, "w") as f:
        ...
        if args.detection_data == "observed":
            f.attrs["detection_rule"] = (
                "gmd._detect_on_observation: SNR of ONE recorded measurement "
                ">= snr_threshold (PR #334/#335 observed-data rule, no "
                "projection latent)")
        else:
...
def parse_args(argv=None):
    ...
    p.add_argument("--detection_data", choices=["true", "observed"], default="true",
                   help="Must match the EVENTS' rule. 'true' (default, historical): "
                        "gmd._network_snr with the projection latent. 'observed': "
                        "PR #334/#335 rule -- one measurement per proposal, "
                        "threshold applied to ITS SNR (gmd._detect_on_observation); "
                        "stored coordinates/pdraw remain the true parameters.")
    p.add_argument("--snr_ref", type=float, default=11.5,
                   help="Observed-mode detection amplitude scale (must equal the "
                        "events'; ignored for 'true').")
    p.add_argument("--dL_fractional_uncertainty", type=float, default=0.10,
                   help="Observed-mode measurement model, must match the events'.")
...
def main(argv=None):
    ...
    meta = {
        ...
        "measurement_model": (pe_kwargs if args.detection_data == "observed"
                              else None),
    ...
    if args.detection_data == "observed":
        print(f"  detection           : observed-data rule (PR #334/#335), "
