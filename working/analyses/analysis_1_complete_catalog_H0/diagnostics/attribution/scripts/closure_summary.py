...
# 2026-08-01 reorg: this script now lives in diagnostics/attribution/scripts/.
# It is cross-cutting -- it reads the analysis of record AND this stage's own
# products -- so RES searches the record results/ first and then this stage's.
STAGE = Path(__file__).resolve().parent.parent          # diagnostics/attribution
ROOT = STAGE.parent.parent                              # the analysis directory
RES = ROOT / "results"
RES_STAGE = STAGE / "results"
PRE = ROOT / "attic" / "results_prefix2"
ATTIC = ROOT / "attic" / "results_dsc_attic"
DATA = Path("/hildafs/projects/phy230014p/magana/gws-agn/working/data")
TRUTH = 67.74
SEEDS = [100, 101, 102, 103, 105]


#: search order for a bare result name, after the 2026-08-01 reorg: the analysis
#: of record first, then this stage's archive, then the probes stage (the nside
#: study's surveys_nside.json and its ns64/ns128 control scans live there).
RES_DIRS = [RES, RES_STAGE, STAGE.parent / "probes" / "results"]


def rfind(name):
    """First existing copy of ``name`` across RES_DIRS (record wins)."""
    for d in RES_DIRS:
        p = Path(d) / name
        if p.exists():
            return p
...
def scan(tag, where=None):
    d = jload(rfind(f"{tag}.json") if where is None else Path(where) / f"{tag}.json")
    if d is None:
        return None
    h = d["H0"]
...
def main(argv=None):
    ...
    ap.add_argument("--out", default=str(RES_STAGE / "closure_summary.json"))
    ...
    out["h0_single_tracer"] = {"after": jload(rfind("h0_single_tracer.json")),
    ...
    out["closure_seeds_json"] = jload(rfind("closure_seeds.json")) is not None

    # ---------------- the per-event score residual --------------------------
    sc = {}
    for tr in ("gal", "agn"):
        aft = jload(rfind(f"attr_terms_{tr}_s100_postfix.json"))
        bef = jload(rfind(f"attr_terms_{tr}_s100.json"))
    ...
    sky = {}
    for tr in ("gal", "agn"):
        d = jload(rfind(f"attr_sky_oracle_{tr}.json"))
        if d is None:
            continue
        conv = {}
        for lab in ("ap4", "ap8", "sf5", "sf7", "sub4", "sub6", "nz", "nm",
                    "shift", "base"):
            c = jload(rfind(f"attr_sky_oracle_{tr}_conv_{lab}.json"))
            if c:
                conv[lab] = {
        ...
        np_ = np.load(rfind(f"attr_sky_oracle_{tr}.npz"))
        ps = np_["pole_sigma"]
    ...
    # ---------------- the nside curve ---------------------------------------
    ns = {"surveys": jload(rfind("surveys_nside.json")), "scans": {}}
    for case in ("gal", "agn"):
        for n in (32, 64, 128):
