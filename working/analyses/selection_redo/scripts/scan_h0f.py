...
# numpy 2 removed `np.trapz` (renamed `np.trapezoid`). The cluster env is numpy
# 1.26 and J2's is 2.4, and this file marginalises the (H0, f) grid with it in
# eleven places -- all AFTER the grid is computed, so on numpy 2 a task would
# burn its full GPU time and then die in post-processing. Bind once here rather
# than per call site; identical algorithm, the name is the only difference.
...
def parse_args(argv=None):
    ...
    # [sel] completeness estimator.  The 2026-08 re-check
    # (experiments/experiment_dsmaster_4d_recheck) retired "per_pixel", which
    # this campaign's predecessor used: it is the only one of the three that
    # puts the galaxy density anchor outside its own 90% interval (railing at
    # +1.19 dex), and the data disfavour it by ln B 17.7.  "selection" is the
    # mode of record here.  "per_pixel"/"aggregate" stay reachable so a cell
    # can be reproduced against the archived campaign.
    ap.add_argument("--c_mode", default="selection",
                    choices=["selection", "aggregate", "per_pixel"],
                    help="completeness estimator (default: selection)")
    # [sel] one offline magnitude fit per catalog, in --survey_path order.
    # Anchoring is all-or-nothing upstream: a catalog left unanchored would
    # sample its theta flat across the truncation bounds while the other sits
    # ~1e-2 mag from its fit, so the missing-galaxy budget would be set by an
    # unconstrained nuisance.
    ap.add_argument("--selection_fit", default=None, metavar="JSON[,JSON...]",
                    help="c_mode=selection: comma-separated selection-fit "
                         "JSONs, one per catalog in --survey_path order")
    # [a7] Empty-pixel policy. Exposed to VERIFY it is inert here rather than
    # assert it: under FIELD weighting (which c_mode=selection requires) an
    # empty pixel carries zero count weight and is -inf regardless of this
    # setting -- darksirens/redshift/prior.py:767-777, "the volume fallback is
    # a conditional-mode per-pixel construct with no place in the global field
    # density". That matters because the a7 occupancy axis drives the empty
    # fraction from 8% to 85%, and if the policy were live the axis would be
    # measuring policy x occupancy instead of occupancy.
    ap.add_argument("--complete_empty_pixel_policy", default="zero",
                    choices=["zero", "volume"],
                    help="empty-pixel treatment; inert under field weighting "
                         "(exposed so that can be checked, not assumed)")
...
def main(argv=None):
    ...
    opts = SimpleNamespace(
        ...
        shared_spin=True,
        shared_gamma=True,
        complete_empty_pixel_policy=args.complete_empty_pixel_policy,
        catalog_sky_weighting=args.catalog_sky_weighting,
        # [sel] driven explicitly so the estimator is never implicit; the
        # library default is still the legacy "per_pixel".
        c_mode=args.c_mode,
        selection_prior=None,
        selection_fit=None,
    )
    # [sel] per-catalog magnitude fits.  Gated on c_mode so the aggregate and
    # per_pixel paths stay byte-identical to the archived campaign's.
    if args.c_mode == "selection":
        if args.selection_fit is None:
            _sel_paths = [None] * n_catalogs          # wide-open ablation
        else:
            _sel_paths = [e.strip() or None
                          for e in args.selection_fit.split(",")]
        if len(_sel_paths) != n_catalogs:
            sys.exit(f"[fatal] --selection_fit gives {len(_sel_paths)} entries "
                     f"for {n_catalogs} catalogs: pass one fit per catalog in "
                     "--survey_path order (anchoring is all-or-nothing), or "
                     "omit the flag for the wide-open ablation.")
        opts.selection_fit_paths = _sel_paths
    elif args.selection_fit:
        sys.exit("[fatal] --selection_fit is meaningful only with "
                 "--c_mode selection")
    ...
    # [sel] Reads each catalog's fit, builds the truncated-normal theta priors
    # on that catalog's OWN labels (Mstar_hat/alpha, Mstar_hat_c2/alpha_c2),
    # pins each m_lim to its fit's datum, and stamps opts.selection_family.
    # Must precede build_parameter_space, which consumes selection_prior /
    # selection_family -- the same ordering darksirens' own cli/inference.py uses.
    _sel_kw = {}
    if args.c_mode == "selection":
    ...
    res = build_parameter_space(
        ...
        c_mode=opts.c_mode,          # [sel] broadcast to every catalog
        **_sel_kw,
    )
    labels = list(res[0])
    ...
    # [sel] c_mode="selection" adds four LF labels to the free set that the
    # other estimators do not carry (Mstar_hat/alpha per catalog).  PIN them at
    # their anchored prior centres -- i.e. at the offline fits -- exactly as
    # delta/sigma_kde are pinned from NUISANCE_DEFAULTS.
    #
    # Pinning is what keeps this campaign comparable to the archived one: the
    # scanned axes stay (H0, fcat_2) and the fixed anchors stay the anchors, so
    # a difference against the archive is the completeness estimator and not a
    # changed parameter space.  It is also where the truth is -- the fits
    # recover the mock's own LF to <=1.8 sigma at every rung.
    if args.c_mode == "selection":
        for _lbl, (_loc, _sd) in (opts.selection_prior or {}).items():
            point.setdefault(_lbl, float(_loc))
        _pin = {k: point[k] for k in
                ("Mstar_hat", "alpha", "Mstar_hat_c2", "alpha_c2") if k in point}
        print(f"[selection] theta pinned at the fit centres: {_pin}")
    ...
    with h5py.File(h5_path, "w") as f:
        ...
        # [sel] the estimator is the axis this campaign exists to change, so it
        # is stamped explicitly rather than left to be inferred from arg_c_mode.
        f.attrs["c_mode"] = args.c_mode
        if args.c_mode == "selection":
            f.attrs["selection_family"] = str(
                getattr(opts, "selection_family", "") or "")
            f.attrs["selection_fit_paths"] = json.dumps(
                [str(p) for p in (getattr(opts, "selection_fit_paths", []) or [])])
            f.attrs["selection_theta_pinned_at"] = json.dumps(
                {k: float(v[0]) for k, v in
                 (getattr(opts, "selection_prior", None) or {}).items()})
            f.attrs["selection_theta_treatment"] = (
                "PINNED at the anchored prior centres (the offline fits), not "
                "sampled, so the scanned axes match the archived per_pixel "
    ...
    summary = {
        "file": str(h5_path),
        "scan": args.scan,
        # [sel] see the h5 attrs of the same names
        "c_mode": args.c_mode,
        "selection": ({
            "family": getattr(opts, "selection_family", None),
            "fit_paths": [str(p) for p in
                          (getattr(opts, "selection_fit_paths", []) or [])],
            "theta_pinned_at": {k: float(v[0]) for k, v in
                                (getattr(opts, "selection_prior", None)
                                 or {}).items()},
        } if args.c_mode == "selection" else None),
        "labels": labels,
