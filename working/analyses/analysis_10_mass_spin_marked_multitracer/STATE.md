...
**Stage (2026-09-21, later): arms A10-S and A10-M DONE; the A10-J cube (job 1335496, 8 chunks, 2 GPUs, ~22 h) is running; the ONE allowed Δμ_G refinement is ordered.** A10-S (41 cells, 0 rejected): f median 0.2953, 68% [0.2451, 0.3463], 90% [0.2120, 0.3801], MAP 0.300. A10-M (861 cells, 0 rejected, min N_eff/thr 5.58): f 0.2939 [0.2486, 0.3427] / [0.2192, 0.3732], Δμ_G **4.528** [3.608, 5.476] / [3.158, 5.907] Msun, MAP (0.275, +5.0), ρ(f, Δμ_G) = −0.616, both axes contained (Δμ_G edges 1.7e-27 / 1.1e-9 of the peak). The planted +5 lies inside the 68% interval. **Refinement:** sd(Δμ_G) = 0.63 Msun against 1 Msun spacing (fewer than two nodes across the 68% width, peak between nodes 4 and 5) is visibly under-resolved, so six half-integer nodes {2.5, 3.5, 4.5, 5.5, 6.5, 7.5} are added as additive rows to arm M (246 cells) and arm J (246 rows, 15,006 cells, ~12.6 GPU-h), queued behind the main cube; no further refinement. **Scoring note:** the model's f_AGN is the pre-selection host fraction, so its target is the planted 0.30; the detected-set fraction 0.357 is descriptive and is what a model that ignored the branch-dependent detectability would drift toward. Event-level decomposition job 1335498 (four nested models at the provisional node f = 0.350, Δμ_χ = +0.1075, Δμ_G = +5) is queued behind the cube. Previous stage: Gates G, V, B and 15 PASS
