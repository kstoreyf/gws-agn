...
likelihood as a mixture weighted by \fagn, identified by which catalog's
...
fraction. Flux-limited catalogs preserve the measurement when completeness
follows from each catalog's magnitude limit and luminosity function: with
the host densities free, the recovered galaxy density lies inside the 68\%
...
\SeedLnBFmin\ to \SeedLnBFmax\ $e$-folds. A factor-of-two error in that density moves \Hz\ by
\HzDensityShiftFactorTwoDeep~\kmsmpc\ above \CompMTwenty\ per cent
completeness, by \HzDensityShiftFactorTwo\ at \CompMEighteen\ per cent. The branching ratio between AGN-assisted and field mergers is
