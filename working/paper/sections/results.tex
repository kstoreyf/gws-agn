...
repeats this recovery across \ClosureNseeds\ independent realisations. The measurements that
...
alone (Figure~\ref{fig:single}): the second catalog sharpens the expansion
rate rather than diluting it.
\label{fig:joint}}
\end{figure*}
...
input value inside the 90\% interval in \PureAgnInNinety\ and
\PureGalInNinety\ of \PureNseeds\ realisations (Figure~\ref{fig:pure}); the
...
a factor of \PureWidthRatio, and the AGN-over-galaxy half-width formed
realisation by realisation is $\PureWidthRatioPerSeed$. A catalog one hundred
times sparser is not a weaker version of the same prior: per event that
actually belongs to it, it is several times stronger, because each event's
line of sight holds a handful of candidate hosts rather than a smooth crowd.
This is the per-event information behind the reading of
...
catalogs' completeness within the volume the events occupy, moves \fagn\ by
\FagnRelSpan\ from end to end and \Hz\ by \HzeroRelSpan~\kmsmpc\ (a fitted
trend of \HzeroRelSlope~\kmsmpc\ per dex, whose sign is not resolved),
neither span reaching a single fit's own statistical error, on the reference
realisation with both densities held at their input values.
