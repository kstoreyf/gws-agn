...
panels. Gray: the 90\% regions and marginals of four further realisations of
the entire simulated universe. The quoted medians and 68\% marginal intervals
...
its mass. The one bimodal galaxy realisation, annotated in that panel
(second mode at \PureBimodalModeLo~\kmsmpc\ at
\PureBimodalHeight\ of the primary mode at \PureBimodalModeHi), is drawn as it
is. Right: the same ten
measurements as medians with 90\% intervals against the same input line,
the two tracers offset slightly from each other at each realisation for
legibility, with each tracer's mean offset over the realisations and the
90\% interval on the mean offset as line and band. At equal event count, the
catalog one hundred times sparser is a factor of \PureWidthRatio\ sharper.
\label{fig:pure}}
\end{figure*}
...
$\PureGalOffset$ from the galaxy sets, each consistent with zero, with the
input value inside the 68\% interval in \PureAgnInSixtyEight\ (AGN) and
...
most \PureLaneMaxAgn\ of the corresponding half-width across all twenty
analyses. One galaxy realisation is bimodal, with a second mode at
\PureBimodalModeLo~\kmsmpc\ standing at \PureBimodalHeight\ of the primary
mode at \PureBimodalModeHi, and that second mode is what widens the galaxy
mean offset's standard error; the cross-check on the same events is unimodal.
...
$\PureWidthRatioPerSeed$. A catalog one hundred
