...
analyses of \S\ref{sec:results:incomplete} confirm on the reference
realisation: a one-dex error in the
assumed density moves \fagn\ by \FagnDensitySlope, nearly independently of
survey depth. Hard X-ray census work
already fixes the space density of survey-defined AGN
classes \citep{Ananna2022}; the harder question, which
...
\S\ref{sec:results:incomplete} measure its effect on this same simulated
universe. With the densities anchored, little falls on either parameter:
the input \Hz\
...
at \CompMEighteen\ per cent. With the densities free the cost falls on the
fraction, which the data bound rather than measure, while the galaxy
density is recovered inside the 68\% interval in all three realisations;
the input \Hz\ then lies outside the 68\% interval but inside the 90\% on
the reference realisation. The requirement is on each catalog's absolute
...
\RelCompletenessSpanDex~dex in the ratio of the two completenesses, spreads
the recovered \fagn\ over \FagnRelSpan\ and \Hz\ over \HzeroRelSpan~\kmsmpc,
...
\fagn\ is also the sharpest \Hz\ the complete catalogs deliver. As
