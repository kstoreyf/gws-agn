...
pixel-by-pixel normalisation every occupied pixel would instead carry unit
weight, and the difference between where and how strongly each catalog traces
the density field is lost.
...
\subsection{Completion of flux-limited catalogs}
\label{sec:methods:completion}

A flux-limited survey stops finding hosts beyond some redshift, and the objects
it misses host mergers all the same. We model the missing objects of each
catalog with a smooth comoving density,
\begin{equation}
...
$n_{0,k} \rightarrow 0$ leaves nothing missing and recovers the
...
in \nzagn\ moves the AGN prior's out-of-catalog mass and \fagn\ follows.

Equation~(\ref{eq:completeness}) measures the completeness from the observed
counts against the assumed density; we call this the number-count
completeness. When the survey's selection is known, the completeness can
instead be predicted from it, with no reference to the counts: an
apparent-magnitude limit $m_{\rm lim}$ acting on a catalog whose absolute
magnitudes follow a luminosity function $\phi(M)$ gives
$C_k(z) = \int^{M_{\rm lim}(z)} \phi(M)\,\dd M \,/ \int \phi(M)\,\dd M$, with
$M_{\rm lim}(z)$ the absolute magnitude that reaches the limit from that
redshift; we call this the luminosity-function completeness. We fit a
Schechter $\phi$ \citep{Schechter1976} to each catalog at each limit, on the
catalog's true redshifts so that the sharp truncation at the limit is not
smeared by the photometric scatter, and hold the fit fixed across
realisations; the luminosity function is thereby treated as known. The two
evaluations need not agree on a flux-limited catalog, where the counts are
themselves the quantity the completeness corrects;
\S\ref{sec:results:incomplete} lets the data adjudicate between them. In the
complete-catalog analyses of this paper $n_{0,k}$ is taken to that limit;
the flux-limited analyses use the luminosity-function completeness, with
$(n_{0,k}, \delta_k)$ anchored at the simulation's input densities where
stated and free otherwise.
...
third component uses the same pixelated catalog the likelihood
conditions on, so proposal and target cannot disagree through a pixelisation or
kernel-width mismatch. The injection sets are drawn once, against the
complete catalogs, and reused at every survey depth.

\subsection{Priors and inference}
\label{sec:methods:inference}

With \Hz\ and \fagn\ the only free parameters, we evaluate
Equation~(\ref{eq:like}) on a grid rather than sampling it; where the host
densities are free as well we sample it by nested sampling
\citep{Speagle2020}, which returns the evidence along with the posterior. Priors are flat, and
...
values, so the mass distribution is informative but not free. \Hz\ is scanned
over $[\HzeroScanLo, \HzeroScanHi]$~\kmsmpc, and where the host densities are
free they are scanned over $[\DensityScanGalLo, \DensityScanGalHi]$
(galaxies) and $[\DensityScanAgnLo, \DensityScanAgnHi]$ (AGN) in
$\log_{10} n_0$, ranges that hold both input values in their interiors.
