...
One simulated universe carries the measurements of \S\ref{sec:results}: a
clustered two-tracer host catalog, a population of mergers placed on that
catalog and observed once each, and flux-limited versions of the same catalog.
Figure~\ref{fig:pgm} is the graph of the generative model.

\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{figures/fig_pgm.pdf}
\caption{The generative model of \S\ref{sec:data}. Shaded single-ringed nodes
are observed, open nodes are latent, small filled squares are fixed
hyperparameters, and double-ringed nodes are deterministic functions of their
parents, drawn shaded whether or not they are observed: the noise-free
amplitude $\rho_i$ is not. The cosmology
...
limit $m_{\rm lim}$ thins $C_k$ into the flux-limited catalog $\hat{C}_k$, the
only version the flux-limited analyses see. The event plate
($i = 1 \dots \Nobs$)
...
amplitude $\hat{\rho}_i$ (written $\rho_{{\rm obs},i}$ in
\S\ref{sec:data:events}) carries the measurement noise. Everything
downstream is a function of observed data, never of the latent source: the
detection indicator $D_i = \Theta(\hat{\rho}_i - \rho_{\rm th})$, every
measurement width, the observed direction $\hat{n}_i$, and the
...
HEALPix \citep{Gorski2005} grid with $N_{\rm side} = \FieldNside$ and $\ell_{\rm max} =
...
the $B$-band Schechter \citep{Schechter1976} function used to characterise the GLADE catalogs
...
catalogs returns a bias ratio of \BiasRatioMeas\ against the input
\BiasRatio, so the clustering contrast that
...
scatter actually present (the normalised residuals have a standard deviation
of \CatPhotozPullSd\ in the galaxy catalog and \CatPhotozPullSdAgn\ in the AGN
catalog). Mergers occur at the true redshift; the survey does not know it
exactly.
...
Each source is measured once, and that measurement is the event. A single
scalar stands in for the network signal-to-noise ratio; its noise-free value depends
...
Flux-limited catalogs are produced by applying an apparent-magnitude limit
...
Pairing one tracer's catalog with the other's at a different limit gives two
surveys of different depth at unchanged number density, which is what lets
\S\ref{sec:results:incomplete} tell the depth of a survey apart from the
density of what it counts.
