...
# Analysis 3's worker with the chunk machinery dropped (no complete-pair grids
# here) and the task line widened to carry the arm's density.
#
# Task line:  SEED GLEV ALEV LANE ARM N0C2
#
...
# after `rm -rf queue/claim_*`.  Workers price each candidate from analysis 3's
# measured per-rung s/eval and claim only what their remaining allocation covers.
set -uo pipefail
cd "$(dirname "$0")/.."
Q=queue
N=$(wc -l < "$Q/tasks.txt")
...
while true; do
  ...
  for i in $(seq 1 "$N"); do
    ...
    read -r SEED GLEV ALEV LANE ARM N0C2 < <(sed -n "${i}p" "$Q/tasks.txt")
  ...
  read -r SEED GLEV ALEV LANE ARM N0C2 < <(sed -n "${CLAIM}p" "$Q/tasks.txt")
  ...
  if SEED=$SEED GLEV=$GLEV ALEV=$ALEV LANE=$LANE ARM=$ARM N0C2=$N0C2 \
      ./scripts/run_scan.sh; then
    touch "$Q/claim_$CLAIM/done"
