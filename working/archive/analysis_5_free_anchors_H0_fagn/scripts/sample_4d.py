...
A4_SCRIPTS = (HERE.parent.parent / "analysis_4_density_anchoring_H0_fagn" / "scripts")
...
def build_closure(args):
    ...
    return likelihood, labels, base, idx


# --------------------------------------------------------------------------- #
# wiring validation: reproduce stored grid logL values pointwise
# --------------------------------------------------------------------------- #
def wiring_check(likelihood, base, idx, refs, tol):
    import h5py
    worst = 0.0
    for ref in refs:
        ...
        for (ih, jf) in cells:
            ...
            print(f"[wiring] {Path(ref).name} H0={H0g[ih]:.2f} f={fg[jf]:.3f} "
                  ...
                  f"|diff|={diff:.3e}")
    assert worst <= tol, (f"[fatal] wiring check FAILED: max|diff|={worst:.3e} "
                          f"> tol={tol:.1e}")
...
def main(argv=None):
    ...
    likelihood, labels, base, idx = build_closure(args)
