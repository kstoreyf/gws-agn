#!/usr/bin/env python3
"""Config-driven conditional/joint logL scan driver for the darksirens likelihood.

Builds the darksirens likelihood *closure* by module import following the proven
`scan_fcat_conditional.py` recipe (load_all_data -> validate_loaded_survey_shapes
-> build_parameter_space -> get_fixed_population_params -> make_likelihood), then
scans the requested axis/axes (H0, fcat_2, or the H0x fcat_2 joint) at a fixed
nuisance point.  The closure is the *pure* likelihood (no prior); scan values are
physical coordinates in `labels` order, passed straight to `float(likelihood(coord))`.

Outputs `<outdir>/<out_tag>.h5` (grids + logL array + full provenance attrs) and
`<outdir>/<out_tag>.json` (flat-prior posterior summary in the `summarize_grid.py`
style: MAP, median, equal-tailed 68/90% CIs, trapezoid marginals, and, for joint
scans, the Gaussian-approx correlation rho from the normalized 2-D posterior).

Device / env note: JAX_PLATFORMS and XLA_PYTHON_CLIENT_PREALLOCATE are set BEFORE
importing darksirens (i.e. before JAX initializes its backend).

Example (K=2 mixture, conditional fcat_2 scan through the multitracer truth point):

  python scan_darksirens.py \
      --universe_model dark_sirens \
      --survey_path .../gal.h5 .../agn.h5 \
      --gw_path .../mock_gw_events.h5 --gwselection_path .../mock_gw_selection.h5 \
      --scan f --f_grid 0 1 41 --h0_fixed 67.74 \
      --log10n0 -3.5290628 --log10n0_c2 -5.5290628 \
      --out_tag mock_fscan --device cpu
"""
import argparse
import json
import os
...
MERGE_SHA = "d387b4f"  # PR #195 (K-catalog mixture) — must be an ancestor of HEAD
...
def parse_args(argv=None):
    ...
    ap.add_argument("--selection_neff_guard", choices=["auto", "hard", "soft"],
                    default="auto",
                    help="Sparse-selection Neff guard mode (darksirens-accepted values).")
...
def main(argv=None):
    ...
    if n_catalogs == 2 and args.universe_model != "dark_sirens":
        sys.exit("[fatal] K=2 mixture requires --universe_model dark_sirens "
                 "(core.py:224 guard; got {!r}).".format(args.universe_model))
